...
public class RectTool implements Tool {
    ...
    @Override public void onDragged(MouseEvent e, ImageDocument doc) {
        cx = e.getX(); cy = e.getY();
        boolean square = e.isShiftDown(); // Hold Shift for square
        double x = Math.min(sx, cx);
        double y = Math.min(sy, cy);
        double w = Math.abs(cx - sx);
        double h = Math.abs(cy - sy);
        if (square) {
            double m = Math.max(w, h); w = h = m; if (cx < sx) x = sx - m; if (cy < sy) y = sy - m;
        }
    ...
    @Override public void onReleased(MouseEvent e, ImageDocument doc) {
        ...
        double w = Math.abs(cx - sx);
        double h = Math.abs(cy - sy);
        if (square) { double m = Math.max(w, h); w = h = m; if (cx < sx) x = sx - m; if (cy < sy) y = sy - m; }
